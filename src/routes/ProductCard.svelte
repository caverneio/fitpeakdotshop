<script lang="ts">
	...
	export let stock: boolean;
</script>

<div class="bg-white flex p-6 font-mono w-max border" {id}>
	<div
	...
	<form class="flex-auto pl-6">
		...
		>
			<h1 class="relative w-full flex-none mb-2 text-2xl font-semibold text-white">{name}</h1>
			<div class="relative text-lg text-white">PEN {price.toFixed(2)}</div>
			<div class="relative uppercase text-teal-400 ml-3">{stock ? 'En Stock' : 'Agotado'}</div>
		</div>
		...
		<p class="text-xs leading-6 text-slate-500">Envío gratuito a partir de PEN 120.</p>
	</form>
</div>
